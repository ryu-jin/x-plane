	...
	if radio_commands_active then
		if not last_button(var_com1) and button(var_com1) then
			...
			if active_com ~= var_com1 then
				sound = load_WAV_file("Resources/plugins/FlyWithLua/sounds/com1tr.wav")
				play_sound(sound)
		...
		if (button(var_com2)) then
			...
			set_button_assignment( (4*40) + 4, "sim/radios/stby_com2_fine_up" )
			set_button_assignment( (4*40) + 5, "sim/radios/stby_com2_fine_down" )
			if active_com ~= var_com2 then
				sound = load_WAV_file("Resources/plugins/FlyWithLua/sounds/com2tr.wav")
				play_sound(sound)
		...
		if not last_button(var_nav_switch) and button(var_nav_switch) then
			if var_nav_active==2 then 
				...
				var_nav_active = 1
				sound = load_WAV_file("Resources/plugins/FlyWithLua/sounds/nav1a.wav")
				play_sound(sound)
			...
			elseif var_nav_active==1 then  
				...
				set_button_assignment( (12*40) + 11, "sim/radios/obs2_down" )
				set_button_assignment( (12*40) + 12, "sim/radios/obs2_up" )					
				var_nav_active = 2
				sound = load_WAV_file("Resources/plugins/FlyWithLua/sounds/nav2a.wav")
				play_sound(sound)
		...
		if not last_button(var_com_switch) and button(var_com_switch) then
			if var_com_active==2 then 
				...
				set_button_assignment( (4*40) + 5, "sim/radios/stby_com1_fine_down" )
				var_com_active = 1
				sound = load_WAV_file("Resources/plugins/FlyWithLua/sounds/com1a.wav")
				play_sound(sound)
			...
			elseif var_com_active==1 then  
				...
				set_button_assignment( (4*40) + 5, "sim/radios/stby_com2_fine_down" )
				var_com_active = 2
				sound = load_WAV_file("Resources/plugins/FlyWithLua/sounds/com2a.wav")
				play_sound(sound)
